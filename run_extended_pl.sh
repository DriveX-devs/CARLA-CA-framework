#!/usr/bin/env bash
# Packet-loss variant of run_extended.sh (which is left untouched).
#
# Identical to CA/run_extended.sh in every respect except that the ns-3
# v2x-bridge is started with two extra command-line options:
#
#   --injected-loss-perc=$INJECTED_LOSS_PERC   (env INJECTED_LOSS_PERC, default 0)
#   --RngRun=$BRIDGE_RNG_RUN                   (env BRIDGE_RNG_RUN,     default 1)
#
# The injected loss is applied by the bridge at the destination application
# socket: the packet really traverses the simulated air interface (so radio
# load and the latency distribution are unchanged) and is then discarded and
# replied as "lost" instead of being handed back to the application.  The
# python application (CA/run_ca_extended.py) is used unmodified: it already
# logs a non-delivered packet with its bridge status in v2x_packets.csv.
#
# --RngRun selects the loss realisation: the drop draws come from a dedicated
# UniformRandomVariable created before any model object, so the sequence
# depends on --RngRun/--RngSeed alone.
#
...
INJECTED_LOSS_PERC=${INJECTED_LOSS_PERC:-0}
BRIDGE_RNG_RUN=${BRIDGE_RNG_RUN:-1}
...
if [ "$APP_MODE" -ge 2 ]; then
    ...
    echo "Starting the v2x-bridge: maxUes=${MAXUES}, gNB (${GNB_X}, ${GNB_Y})," \
         "injected-loss=${INJECTED_LOSS_PERC}%, RngRun=${BRIDGE_RNG_RUN}," \
         "log ${BRIDGE_LOG}"
    LD_LIBRARY_PATH="$NS3_BUILD_DIR/lib" setsid "$BRIDGE_BIN" \
        --maxUes="$MAXUES" --gnbX="$GNB_X" --gnbY="$GNB_Y" \
        --listenPort="$BRIDGE_PORT" \
        --injected-loss-perc="$INJECTED_LOSS_PERC" \
        --RngRun="$BRIDGE_RNG_RUN" \
